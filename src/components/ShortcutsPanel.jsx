...
export const ShortcutsPanel = ({ isOpen, onClose }) => {
  ...
  // Trap focus inside the panel securely
  useEffect(() => {
    if (!isOpen) return;
    ...
    const handleKeyDown = (e) => {
      ...
      // --- 🟢 REAL MAANG-GRADE FOCUS TRAP ---
      if (e.key === "Tab") {
        // If there are multiple buttons later, you querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')
        // For now, since Close is the only focusable element, we just lock them to it.
        e.preventDefault();
