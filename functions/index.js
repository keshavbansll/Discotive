...
exports.exportUserData = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      // Initialize the compliance payload (Execution Map Purged)
      const exportData = {
      ...
      const [journalSnap, scoreSnap, verifySnap] = await Promise.all([
...
exports.processColistResonanceMilestones = onSchedule(
  ...
  async () => {
    ...
    // This reduces hourly reads from O(N_Total) to O(N_Active), saving 99% on database costs.
